...
CMPD_SUMBOLS = {"compound", "cmpd", "id"}

PLACEHOLDER_ATOMS = ["Lv", "Lu", "Nd", "Yb", "At", "Fm", "Er"]

POSITIONS = {'o': 'ortho-', 'm': 'meta-', 'p': 'para-'}

ISOMERISMS = {'i': 'iso-', 's': 'sec-', 't': 'tert-', 'n': '', 'c': 'cyclo-'}  # 异构体，其中norm 一般应省略

PREFIXES = {**POSITIONS, **ISOMERISMS}
...
FORMULA_REGEX = re.compile('(' + '|'.join(list(ABBREVIATIONS.keys())) + '|R[0-9]*|[A-Z][a-z]+|[A-Z]|[0-9]+|\(|\))')
