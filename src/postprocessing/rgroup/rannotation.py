import logging
from collections import Counter, defaultdict
from src.postprocessing.rgroup.schema import RGroupAbbr, Compound
from src.postprocessing.rgroup.entity import RGroup
import re

log = logging.getLogger(__name__)

ASSIGNMENT = ':=-'
SEPERATORS = ','
CONCENTRATION = '%()<>'
BLACKLIST_CHARS = ASSIGNMENT + SEPERATORS + CONCENTRATION
# Regular Expressions
NUMERIC_REGEX = re.compile('^\d{1,3}$')
ALPHANUMERIC_REGEX = re.compile('^((d-)?(\d{1,2}[A-Za-z]{1,2}[′″‴‶‷⁗]?)(-d))|(\d{1,3})?$')

# Commonly occuring tokens for R-Groups:
r_group_indicators = ['R', 'X', 'Y', 'Z', 'R1', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7', 'R8', 'R9', 'R10', 'Y2', 'D', "R'",
                      "R''", "R'''", "R''''"]
r_group_indicators = r_group_indicators + [val.lower() for val in r_group_indicators]

SPLIT = [
    ' ',  # Specific whitespace characters
    ' ',
    '----',
    '––––',  # \u2013 en dash
    '————',  # \u2014 em dash
    '<--->',
    '---',
    '–––',  # \u2013 en dash
    '———',  # \u2014 em dash
    '<-->',
    '-->',
    '...',
    '--',
    '––',  # \u2013 en dash
    '——',  # \u2014 em dash
    '``',
    # "''",
    '->',
    '<',
    '>',
    '–',  # \u2013 en dash
    '—',  # \u2014 em dash
    '―',  # \u2015 horizontal bar
    '~',  # \u007e Tilde
    '⁓',  # \u2053 Swung dash
    '∼',  # \u223c Tilde operator
    '°',  # \u00b0 Degrees
    ';',
    ':',
    '@',
    '#',
    '$',
    '£',  # \u00a3
    '€',  # \u20ac
    '%',
    '&',
    '?',
    '!',
    '™',  # \u2122
    '®',  # \u00ae
    '…',  # \u2026
    '⋯',  # \u22ef Mid-line ellipsis
    '†',  # \u2020 Dagger
    '‡',  # \u2021 Double dagger
    '§',  # \u00a7 Section sign
    '¶'  # \u00b6 Pilcrow sign
    '≠',  # \u2260
    '≡',  # \u2261
    '≢',  # \u2262
    '≣',  # \u2263
    '≤',  # \u2264
    '≥',  # \u2265
    '≦',  # \u2266
    '≧',  # \u2267
    '≨',  # \u2268
    '≩',  # \u2269
    '≪',  # \u226a
    '≫',  # \u226b
    '≈',  # \u2248
    '=',
    '÷',  # \u00f7
    '×',  # \u00d7
    '→',  # \u2192
    '⇄',  # \u21c4
    # '"',  # \u0022 Quote mark
    # '“',  # \u201c
    # '”',  # \u201d
    '„',  # \u201e
    # '‟',  # \u201f
    # '‘',  # \u2018 Left single quote
    # '’',  # \u2019 Right single quote  - Regularly used as an apostrophe, so don't always split
    '‚',  # \u201a Single low quote
    ',',
    # '‛',  # \u201b Single reversed quote
    # '`',  # \u0060
    # '´',  # \u00b4
    # Brackets
    '[',
    '{',
    '}',
    ']',
    '+',  # \u002b Plus
    '±',  # \u00b1 Plus/Minus
]


class Token:
    def __init__(self, text, start, end):
        self.text = text
        self.start = start
        self.end = end

    def __repr__(self):
        return f"Token(text='{self.text}', start={self.start}, end={self.end})"


class Sentence:
    def __init__(self, raw_text: str):
        self.raw = self._remove_specific_parentheses(raw_text)
        self.tokens = self._tokenize()

    def _tokenize(self):
        tokens = []
        i = 0
        while i < len(self.raw):
            # 跳过空白字符
            if self.raw[i].isspace():
                i += 1
                continue

            matched = False
            for sep in SPLIT:
                if self.raw.startswith(sep, i):
                    # 添加分割词 Token
                    token_end = i + len(sep)
                    tokens.append(Token(sep, i, token_end))
                    i = token_end
                    matched = True
                    break
            if not matched:
                # 收集普通字符直到遇到下一个分割词
                start = i
                while i < len(self.raw):
                    found_sep = False
                    for sep in SPLIT:
                        if self.raw.startswith(sep, i):
                            found_sep = True
                            break
                    if found_sep:
                        break
                    i += 1
                if start < i:
                    tokens.append(Token(self.raw[start:i], start, i))
        return tokens

    def _remove_specific_parentheses(self, raw_text):
        """
        如果括号内包含 '=', ',' 或 'R'，则只删除括号本身，保留其内容。

        参数:
            text (str): 输入字符串。

        返回:
            str: 移除指定括号后的字符串。
        """
        # 这个正则表达式寻找：
        # \(          -> 一个字面意义上的左括号
        # ([^()]* -> 捕获组1：匹配任何非括号字符，零次或多次
        # [=,R]       -> 后面跟着 '=', ',' 或 'R' 中的任意一个
        # [^()]* -> 继续匹配任何非括号字符，零次或多次
        # )           -> 捕获组1结束
        # \)          -> 一个字面意义上的右括号
        # 替换时使用 '\1' 来引用捕获组1的内容，这样就只移除了括号本身。
        pattern = r'\(([^()]*[=,R][^()]*)\)'
        return re.sub(pattern, r'\1', raw_text)


def build_compounds(r_groups):
    # print(r_groups)
    # 调试：输出 label_candidates 为空的情况
    empty_label_rgroups = [rg for rg in r_groups if len(rg.label_candidates) == 0]
    if empty_label_rgroups:
        log.debug("以下 RGroup 的 label_candidates 为空，可能需要检查数据：")
        for rg in empty_label_rgroups:
            log.debug(f"Var: {rg.var}, Value: {rg.value}")

    # 分组处理
    compound_dict = defaultdict(list)

    for rg in r_groups:
        label = rg.label_candidates[0].text if rg.label_candidates else "-"  # 空列表时默认视为统一到一个空字符串标签下
        compound_dict[label].append(rg)

    # 构建 Compound 数组
    compounds = []

    for label, group_list in compound_dict.items():
        r_group_dict = {}

        for rg in group_list:
            if rg.var.text in r_group_dict:
                log.debug(f"重复的 var '{rg.var.text}' 出现在同一化合物中。")
                continue

            # 创建 RGroupAbbr 实例
            rgroup_abbr = RGroupAbbr(
                abbr=rg.value.text
            )
            r_group_dict[rg.var.text] = rgroup_abbr

        # 创建 Compound
        compound = Compound()
        compound.cmpd_id = label or "-"  # 空 label 使用 "-"
        compound.r_groups = r_group_dict
        compounds.append(compound)

    return compounds


def detect_r_group(sentences):
    """ Determines whether a label represents an R-Group structure, and if so gives the variable and value.

    :param diag: Diagram object to search for R-Group indicators
    :return diag: Diagram object with R-Group variable and value candidates assigned.
    """
    output = []
    if not sentences:
        return []
    first_sentence_tokens = [token.text.replace(' ', '').replace('\n', '') for token in sentences[0].tokens]
    # Identifies grid labels from the presence of only variable tokens in the first line
    if all([True if token in r_group_indicators else False for token in first_sentence_tokens]):
        try:
            output = resolve_r_group_grid(sentences)
        except Exception:
            output = []

    # Otherwise looks for indicative R-Group characters (=, :)
    if not output:

        for sentence in sentences:

            all_sentence_text = [token.text for token in sentence.tokens]

            if '=' in all_sentence_text:
                var_value_pairs = detect_r_group_from_sentence(sentence, output, indicator='=')
            elif ':' in all_sentence_text:
                var_value_pairs = detect_r_group_from_sentence(sentence, output, indicator=':')
            else:
                var_value_pairs = []
            # Process R-group values from '='
            r_groups = get_label_candidates(sentence, var_value_pairs)

            r_groups = filter_repeated_labels(r_groups)

            filtered_r_groups = filter_label_candidates(r_groups)

            output.extend(filtered_r_groups)

    cmpds = build_compounds(output)

    return cmpds


def filter_label_candidates(r_groups):
    """
    For each RGroup, retain only the Token with the earliest start position in label_candidates.

    :param r_groups: List of RGroup objects
    :return: Updated list of RGroup objects with filtered label_candidates
    """
    for r_group in r_groups:
        # 如果 label_candidates 为空，跳过
        if not r_group.label_candidates:
            continue

        # 找到起始位置最小的 Token
        earliest_token = min(r_group.label_candidates, key=lambda token: token.start)

        # 更新 label_candidates 为只包含这个 Token
        r_group.label_candidates = [earliest_token]

    return r_groups


def convert_rgroups_to_dict(rgroup_lists):
    """
    Converts a list of RGroup lists into a list of dictionaries.
    If variable names or values contain superscript or subscript numbers (e.g., R¹, R₀), they are converted to regular numbers (e.g., R1, R0).
    If a value is another variable, it is recursively replaced with the corresponding value.
    If a variable name contains only numbers, "R" is automatically added before the numbers.

    :param rgroup_lists: List of lists containing RGroup objects.
    :return: List of dictionaries with variable-value pairs and 'id'.
    """

    # 定义一个辅助函数，用于将上标和下标数字转换为普通数字
    def convert_superscript_and_subscript_to_regular(text):
        superscript_map = {
            '⁰': '0', '¹': '1', '²': '2', '³': '3', '⁴': '4',
            '⁵': '5', '⁶': '6', '⁷': '7', '⁸': '8', '⁹': '9'
        }
        subscript_map = {
            '₀': '0', '₁': '1', '₂': '2', '₃': '3', '₄': '4',
            '₅': '5', '₆': '6', '₇': '7', '₈': '8', '₉': '9'
        }
        combined_map = {**superscript_map, **subscript_map}
        return ''.join(combined_map.get(char, char) for char in text)

    def resolve_variable(value, rgroup_dict):
        """
        Recursively resolve variables in the value.
        If the value is another variable, replace it with the corresponding value.
        """
        while value in rgroup_dict:
            value = rgroup_dict[value]
        return value

    result = []

    # 遍历每个内部列表
    for rgroup_list in rgroup_lists:
        # 每个内部列表生成一个独立的字典
        rgroup_dict = {}
        for rgroup in rgroup_list:
            variable = rgroup.var.text
            value = rgroup.value.text
            # 提取 label_candidates 中 Token 的 text 作为 id
            if rgroup.label_candidates:
                id_value = rgroup.label_candidates[0].text
            else:
                id_value = None

            # 将 variable 和 value 添加到字典
            rgroup_dict['id'] = id_value

            # 如果变量名或值包含上标或下标数字，进行转换
            variable = convert_superscript_and_subscript_to_regular(variable)
            value = convert_superscript_and_subscript_to_regular(value)

            # 如果变量名仅包含数字，自动添加 R 前缀
            if re.match(r'^\d+$', variable):
                variable = f"R{variable}"

            rgroup_dict[variable] = value

        # Resolve any variables in the values
        for key in list(rgroup_dict.keys()):
            if key != 'id':
                resolved_value = resolve_variable(rgroup_dict[key], rgroup_dict)
                rgroup_dict[key] = resolved_value

        # 将生成的字典添加到结果列表
        if rgroup_dict:
            result.append(rgroup_dict)

    return result


def detect_r_group_from_sentence(sentence, normol_output, indicator='='):
    """ Detects an R-Group from the presence of an input character

     :param sentence: A chemdataextractor.doc.text.Sentence object containing tokens to be probed for R-Groups
     :param indicator: String used to identify R-Groups

     :return var_value_pairs: A list of RGroup objects, containing the variable, value and label candidates
     :rtype: List[chemschematicresolver.model.RGroup]
     """

    var_value_pairs = []  # Used to find variable - value pairs for extraction
    for i, token in enumerate(sentence.tokens):
        if token.text is indicator:
            log.debug('Found R-Group descriptor %s' % token.text)
            if i > 0:
                log.debug('Variable candidate is %s' % sentence.tokens[i - 1])
            if i < len(sentence.tokens) - 1:
                log.debug('Value candidate is %s' % sentence.tokens[i + 1])

            if 0 < i < len(sentence.tokens) - 1:
                variable = sentence.tokens[i - 1]
                # 基团值，处理IUPAC命名可能存在的空格，尝试寻找一个yl结束的字符串（认为这样的字符串词数不超过3）
                value_tail_token_idx = i + 1
                for j in range(i+1, len(sentence.tokens)):
                    if sentence.tokens[j].text.endswith('yl'):
                        value_tail_token_idx = j
                        break
                if value_tail_token_idx > i + 1:
                    value = Token(text='-'.join(t.text for t in sentence.tokens[i+1: value_tail_token_idx+1]),
                                  start=sentence.tokens[i+1].start, end=sentence.tokens[value_tail_token_idx].end)
                else:
                    value = sentence.tokens[i + 1]
                var_value_pairs.append(RGroup(variable, value, []))

        elif token.text == 'or' and var_value_pairs:
            or_pair = []
            log.debug('"or" keyword detected. Assigning value to previous R-group variable...')

            # Identify the most recent var_value pair
            variable = var_value_pairs[-1].var
            value = sentence.tokens[i + 1]
            or_pair.append(RGroup(variable, value, []))
            normol_output.extend(or_pair)

    return var_value_pairs


def resolve_r_group_grid(sentences):
    if len(sentences) < 2 or len(sentences[0].tokens) != len(sentences[1].tokens):
        return []
    grid_output = []
    """Resolves the special grid case, where data is organised into label-value columns for a specific variable.

        Please note that this only extracts simple tables, where the column indicators are contained in the list of
        r_group_indicators

    :param sentences: A chemdataextractor.doc.text.Sentence objects containing tokens to be probed for R-Groups
    :return var_value_pairs: A list of RGroup objects, containing the variable, value and label candidates
    :rtype: List[chemschematicresolver.model.RGroup]
    """

    # Used to find variable - value pairs for extraction
    table_identifier, table_rows = sentences[0], sentences[1:]


    variables = table_identifier.tokens
    log.debug('R-Group table format detected. Variable candidates are %s' % variables)

    # Check that the length of all table rows is the same as the table_identifier + 1
    correct_row_lengths = [True for row in table_rows if len(row.tokens) == len(variables) + 1]
    if not all(correct_row_lengths):
        return []

    for row in table_rows:
        tokens = row.tokens
        label_candidates = [tokens[0]]
        values = tokens[1:]
        var_value_pairs = []
        for i, value in enumerate(values):
            var_value_pairs.append(RGroup(variables[i], value, label_candidates))
        grid_output.extend(var_value_pairs)

    return grid_output


def get_label_candidates(sentence, r_groups, blacklist_chars=BLACKLIST_CHARS, blacklist_words=('or',)):
    """Assign label candidates from a sentence that contains known R-Group variables

    :param sentence: Sentence to probe for label candidates
    :param: r_groups: A list of R-Group objects with variable-value pairs assigned
    :param blacklist_chars: String of disallowed characters
    :param blacklist_words: List of disallowed words

    :return r_groups: List of R-Group objects with assigned label candidates
    """

    # Remove irrelevant characters and blacklisted words
    candidates = [token for token in sentence.tokens if token.text not in blacklist_chars]
    candidates = [token for token in candidates if token.text not in blacklist_words]

    r_group_vars_and_values = []
    for r_group in r_groups:
        r_group_vars_and_values.append(r_group.var)
        r_group_vars_and_values.append(r_group.value)

    candidates = [token for token in candidates if token not in r_group_vars_and_values]

    r_groups = assign_label_candidates(r_groups, candidates)

    return r_groups


def assign_label_candidates(r_groups, candidates):
    """ Gets label candidates for cases where the same variable appears twice in one sentence.
        This is typically indicative of cases where 2 R-Groups are defined on the same line
    """

    var_text_counts = Counter(r_group.var.text for r_group in r_groups)
    duplicate_r_groups = [r_group for r_group in r_groups if var_text_counts[r_group.var.text] > 1]

    # Check that ALL r_group values have this duplicity (ie has every r_group got a duplicate variable?)
    if len(duplicate_r_groups) == len(r_groups) and len(r_groups) != 0:

        # Now go through r_groups getting positions of tokens
        for i, r_group in enumerate(r_groups):
            if i == 0:
                end_index = r_group.var.end
                r_group.label_candidates = [cand for cand in candidates if cand.start < end_index]
            elif i == len(r_groups) - 1:
                start_index = r_groups[i - 1].value.end
                end_index = r_group.var.end
                r_group.label_candidates = [cand for cand in candidates if
                                            (start_index < cand.start < end_index) or cand.start > r_group.value.end]
            else:
                start_index = r_groups[i - 1].value.end
                end_index = r_group.var.end
                r_group.label_candidates = [cand for cand in candidates if start_index < cand.start < end_index]

            # 新增，需验证稳定性
            if i > 0 and not r_group.label_candidates:
                r_group.label_candidates = r_groups[i - 1].label_candidates
        return r_groups

    else:
        for r_group in r_groups:
            var = r_group.var
            value = r_group.value
            label_cands = [candidate for candidate in candidates if candidate not in [var, value]]
            r_group.label_candidates = label_cands

        return r_groups


def filter_repeated_labels(r_groups):
    """
     Detects repeated variable values.
     When found, this is determined to be an 'or' case so relative label assignment ensues.

     :param r_groups: Input list of R-Group objects
     :return output_r_groups: R-Group objects corrected for 'or' statements

     """

    or_vars = []
    vars = [r_group.var for r_group in r_groups]
    unique_vars = set(vars)
    for test_var in unique_vars:
        if vars.count(test_var) > 1:
            log.debug('Identified "or" variable')
            or_vars.append(test_var)

    # Get label candidates for r_groups containing this:
    filtered_r_groups = [r_group for r_group in r_groups if r_group.var in or_vars]

    # If no duplicate r_group variables, exit function
    if len(filtered_r_groups) == 0:
        return r_groups

    remaining_r_groups = [r_group for r_group in r_groups if r_group.var not in or_vars]
    label_cands = filtered_r_groups[0].label_candidates  # Get the label candidates for these vars (should be the same)

    # Prioritizing alphanumerics for relative label assignment
    alphanumeric_labels = [label for label in label_cands if ALPHANUMERIC_REGEX.match(label.text)]

    output_r_groups = []

    # First check if the normal number of labels is the same
    if len(filtered_r_groups) == len(label_cands):
        for i in range(len(filtered_r_groups)):
            altered_r_group = filtered_r_groups[i]
            altered_r_group.label_candidates = [label_cands[i]]
            output_r_groups.append(altered_r_group)
        output_r_groups = output_r_groups + remaining_r_groups

    # Otherwise, check if alphanumerics match
    elif len(filtered_r_groups) == len(alphanumeric_labels):
        for i in range(len(filtered_r_groups)):
            altered_r_group = filtered_r_groups[i]
            altered_r_group.label_candidates = [alphanumeric_labels[i]]
            output_r_groups.append(altered_r_group)
        output_r_groups = output_r_groups + remaining_r_groups

    # Otherwise return with all labels
    else:
        output_r_groups = r_groups

    return output_r_groups


def separate_duplicate_r_groups(r_groups):
    """
     Separate duplicate R-group variables into separate lists

     :param r_groups: List of input R-Group objects to be tested for duplicates
     :return output: List of R-Groups with duplicates separated
    """

    if len(r_groups) == 0:
        return r_groups

    # Getting only the variables with unique text value
    vars = [r_group.var for r_group in r_groups]
    vars_text = [var.text for var in vars]
    unique_vars, unique_vars_text = [], []
    for i, var in enumerate(vars):
        if vars_text[i] not in unique_vars_text:
            unique_vars.append(var)
            unique_vars_text.append(vars_text[i])

    var_quantity_tuples = []
    vars_dict = {}
    output = []

    for var in unique_vars:
        var_quantity_tuples.append((var, vars_text.count(var.text)))
        vars_dict[var.text] = []

    equal_length = all(elem[1] == var_quantity_tuples[0][1] for elem in var_quantity_tuples)

    # If irregular, default behaviour is to just use one of the values
    if not equal_length:
        return [r_groups]

    # Populate dictionary for each unique variable
    for var in unique_vars:
        for r_group in r_groups:
            if var.text == r_group.var.text:
                vars_dict[var.text].append(r_group)

    for i in range(len(vars_dict[var.text])):
        temp = []
        for var in unique_vars:
            try:
                temp.append(vars_dict[var.text][i])
            except Exception as e:
                log.error("An error occurred while attempting to separate duplicate r-groups")
                log.error(e)
        output.append(temp)

    # Ensure that each complete set contains all label candidates
    for r_groups_output in output:
        total_cands = []
        for r_group in r_groups_output:
            for cand in r_group.label_candidates:
                total_cands.append(cand)

        for r_group in r_groups_output:
            r_group.label_candidates = total_cands

    return output


def convert_r_groups_to_tuples(r_groups):
    """ Converts a list of R-Group model objects to R-Group tuples"""

    return [r_group.convert_to_tuple() for r_group in r_groups]


if __name__ == '__main__':
    sent = ' a (5a R1 = OMe, R2 = OMe, R3 = Cl)'
    sentences = [Sentence(sent)]
    print('Input:', ' '.join(s.raw for s in sentences))
    res = detect_r_group(sentences)
    print('Output:', res)
