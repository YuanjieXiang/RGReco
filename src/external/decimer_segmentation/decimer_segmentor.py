...
class DECIMERSegmentor:
    def __init__(self, ckpt_path: str, providers=None):
        ...
        self.model = None
        self.config = Config()
        self.load_model(providers)

    def load_model(self, providers):
        self.model = ort.InferenceSession(self.ckpt_path, providers=providers)
    ...
    def predict(self, image):
        ...
        detections, _, _, mrcnn_mask, _, _, _ = self.model.run(
            output_names=["mrcnn_detection", "mrcnn_bbox", "mrcnn_class", "mrcnn_mask", "rpn_class", "ROI", "rpn_bbox"],
            input_feed=inputs
        )
        ...
        return final_rois, final_scores, final_masks
