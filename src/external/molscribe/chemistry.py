...
# Changes include: modified the _expand_functional_group
#
...
def get_smiles_from_symbol(symbol, mol, atom, bonds):
    ...
    if symbol in settings.ABBR_DICT:
        return remove_r_sym_in_smile(settings.ABBR_DICT[symbol])
...
def _expand_functional_group(mol, mappings, debug=False):
    ...
    if _need_expand(mol, mappings):
        ...
        for i in range(num_atoms):
            ...
            if atom.GetSymbol() == '*':
                ...
                if not (isinstance(symbol, str) and len(symbol) > 0):
                    continue
                # r_groups do not need to be expanded, eg. Rg, Rf, Rz
                if (
                    symbol in RGROUP_SYMBOLS or (symbol.startswith("R") and len(symbol) > 1 and not symbol[1].isupper())
                ):  # TODO remove "R" in symbol
                    continue
                ...
                # 处理 sub_smiles 中包含 * 符号的情况
                if "*" in sub_smiles:
                    r_symbol = _find_r_symbol(symbol)
                    if r_symbol:
                        for atom in mol_r.GetAtoms():
                            if atom.GetSymbol() == "*":
                                Chem.SetAtomAlias(atom, r_symbol)
        ...
        for i in atoms_to_remove:
            mol_w.RemoveAtom(i)
        for i in range(mol_w.GetNumAtoms()):
            j = mol_w.GetAtomWithIdx(i).GetNumRadicalElectrons()
            if j > 0:
                print(i, j)

        smiles = convert_mol_to_smiles(mol_w)
...
def _find_r_symbol(s: str) -> str | None:
    """
    在字符串的起始或结束位置查找R-group符号。
    符号只可能出现一个。
    """
...
def _convert_graph_to_smiles(coords, symbols, edges, image=None, debug=False):
    ...
    for i in range(n):
        ...
        if symbol[0] == '[':
            symbol = symbol[1:-1]
        if symbol in RGROUP_SYMBOLS or "R" in symbol:
            atom = Chem.Atom("*")
    ...
    try:
        # 将以下代码插入到 MolScribe/molscribe/chemistry.py 的第 line 549 行，以确保其他元素中特殊手征性的移除。
        for atom in mol.GetAtoms():
            if atom.GetSymbol() != "C":
                atom.SetChiralTag(Chem.rdchem.ChiralType.CHI_UNSPECIFIED)
        ...
        pred_molblock = Chem.MolToMolBlock(mol)  # 不保留原始基团
        success = True
    except Exception as e:
        if debug:
...
def convert_mol_to_smiles(mol):
    if mol:
        ...
        smi = Chem.MolToCXSmiles(mol, Chem.SmilesWriteParams(), rdmolfiles.CXSmilesFields.CX_ALL_BUT_COORDS)
        # print(smi)
        return smi
    return ""
