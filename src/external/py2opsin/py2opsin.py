...
def py2opsin(
    ...
    if jar_fpath == "default":
        jar_fpath = Path(__file__).parent / "opsin-cli-2.8.0-jar-with-dependencies.jar"
    ...
    try:
        ...
        if result.stderr:
            err_str = result.stderr.decode(encoding=sys.stderr.encoding, errors='ignore')
            warnings.warn(
        ...
        if type(chemical_name) is str:
            return (
                result.stdout.decode(encoding=sys.stdout.encoding, errors='ignore')
                .replace("\n", "")
                .replace("\r", "")
            )
        else:
        ...
        warnings.warn(f"Unexpected error ocurred! {str(e)}")
        return ""
    finally:
        try:
            os.remove(tmp_fpath)
            ...
            pass
            warnings.warn(f"删除失败 [{tmp_fpath}]: {str(e)}")
